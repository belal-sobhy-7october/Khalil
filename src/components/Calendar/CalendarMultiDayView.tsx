...
export default memo(function CalendarMultiDayView({ currentDate, events, onEditEvent, onCompleteEvent }: CalendarMultiDayViewProps) {
  const { isRTL } = useTranslation();

  const multiDayStart = useMemo(() => currentDate, [currentDate]);
  const multiDayDates = useMemo(() => 
    Array.from({ length: DAYS }, (_, i) => addDaysToDateString(multiDayStart, i)),
  [multiDayStart]);

  const multiDayEvents = useMemo(() => 
    events.filter(e => multiDayDates.includes(e.date)).sort((a, b) => {
      if (a.allDay && !b.allDay) return -1;
      if (!a.allDay && b.allDay) return 1;
      const dayA = multiDayDates.indexOf(a.date);
      const dayB = multiDayDates.indexOf(b.date);
  ...
  [events, multiDayDates]);

  const allDayEventsByDay = useMemo(() => {
    const map: Record<string, CalendarEvent[]> = {};
    multiDayDates.forEach(d => map[d] = []);
    multiDayEvents.filter(e => e.allDay).forEach(e => {
      if (map[e.date]) map[e.date].push(e);
    });
    return map;
  }, [multiDayEvents, multiDayDates]);
  ...
  return (
    <div className="h-[calc(100vh-300px)] min-h-[500px] overflow-x-auto scrollbar-thin">
      <div className="grid grid-cols-[50px_repeat(3,1fr)] min-w-max">
        ...
        {multiDayDates.map((dateStr) => {
          ...
          return (
            <div key={dateStr} className="relative min-w-[140px]">
