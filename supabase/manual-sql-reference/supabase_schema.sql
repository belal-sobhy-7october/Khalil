...
-- Originally: "Run this in the Supabase SQL Editor" to bootstrap a fresh project.
--
-- STATUS: Superseded by ../migrations/20260801000000_baseline_schema.sql, which
-- reproduces this same schema (minus calendar_events, which has its own proper
-- migration) as a tracked migration so `supabase db reset` works on a fresh
-- environment. Kept here for historical/readable reference only — do not run
-- this against a database that already has these tables.
